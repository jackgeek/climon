...
| dash-22 | Terminal emoji / wide-character width fidelity | Loads the xterm.js Unicode 11 addon and activates version `11` so wide emoji (and other two-cell glyphs) advance the cursor by two cells instead of xterm's default Unicode v6 one-cell width. | Emoji-heavy terminal output stays aligned in the dashboard — no swallowed spaces or ghost glyphs. | [manual-tests/terminal-emoji-width.md](manual-tests/terminal-emoji-width.md); `src/web/components/TerminalView.tsx` (`configureTerminalUnicode`); **dev branch** |
