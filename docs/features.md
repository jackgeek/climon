...
|---|---|---|---|---|
| cli-01 | Run any command in a managed session | `climon run [flags] <command> [args…]` runs your command inside a managed pseudo-terminal and registers it as a session; the bare `climon <command>` form is a shorthand for `run`, and `climon command <command>` is an alias (handy when the command's name clashes with a climon subcommand). | Turn any long-running or interactive command into a session you can watch, drive, and get notified about — no wrapper scripts, just prefix it with `climon`. | [manual-tests/phase08-cli.md](manual-tests/phase08-cli.md); `rust/climon-cli/src/args.rs` |
...
| cli-13 | Per-session theme | The `--theme` flag sets a per-session dashboard theme; sessions without one inherit the `dashboard.theme` default live. (`session.color` is a separate accent-colour setting.) | Give each session its own look so you can visually distinguish, say, prod from dev at a glance. | [manual-tests/per-session-theme.md](manual-tests/per-session-theme.md); `rust/climon-cli/src/args.rs`, `dashboard.theme` |
...
| cli-15 | Secure self-update | `climon update` fetches a release manifest, verifies an Ed25519 detached signature, then performs an atomic non-destructive binary swap; a throttled background check + launch banner surface new versions. (The manifest can flag an `aes-256-gcm-scrypt` envelope, but the Rust updater currently ships signature verification only.) | Stay current safely — updates are cryptographically verified and never kill running sessions, so upgrading is risk-free. | [manual-tests/phase10-update.md](manual-tests/phase10-update.md), [manual-tests/release-signing-key-preflight.md](manual-tests/release-signing-key-preflight.md); `rust/climon-update` |
...
| cli-23 | WSL ⇄ Windows bridge (same machine) | `climon link` records each side's `CLIMON_HOME` in `remote.peerHome` (and can enable `feature.wslBridge`); with the bridge on, sessions stream between a WSL distro and Windows onto one shared dashboard. | See your Windows and WSL sessions together in a single dashboard instead of juggling two separate worlds on the same box. | `feature.wslBridge`; [manual-tests/phase14-wsl-bridge-feature-flag.md](manual-tests/phase14-wsl-bridge-feature-flag.md); `rust/climon-remote`, `src/remote/peer.ts` |
...
| cli-25 | Remote host auto-discovery + fan-out | With `remote.enabled`, a devbox scans its signed-in dev tunnel account for tunnels labeled `climon-ingest`, treats those with `hostConnections >= 1` as live, excludes its own derived ingest tunnel, and fans session uplinks out to every discovered host (unioned with any explicit `remote.tunnelId`/`remote.host`). `remote.discover false` opts out. | Point a devbox at your dashboards with a single setting — no copying tunnel ids — and stream sessions to several dashboards at once. | `feature.remotes`, `remote.discover`; [manual-tests/phase18-remote-discovery.md](manual-tests/phase18-remote-discovery.md); `rust/climon-remote/src/discovery.rs`, `rust/climon-remote/src/target_set.rs`, `rust/climon-remote/src/uplink.rs`; **dev branch** |
| cli-26 | Windows-safe binary lifecycle | Windows installs run stable `climon.exe`/`climon-server.exe` stubs that resolve `climon.version`/`climon-server.version` pointers to versioned payloads (`climon-<ver>.dll` loaded in-process, `climon-server-<ver>.exe` launched as a child). Self-updates write new versioned files and atomically flip the pointer instead of overwriting a locked running executable; `climon cleanup` reaps superseded unlocked payloads. Legacy single-`climon.exe` installs migrate in place via a bridge release + headless `install.exe --migrate`. Unix keeps the standard `climon` executable and rename-over update behavior. | Self-update on Windows never blocks on open terminals — running sessions keep the old code until restarted, so upgrading is always safe. | [manual-tests/windows-binary-lifecycle.md](manual-tests/windows-binary-lifecycle.md); `rust/climon-stub`, `rust/climon-dll`, `rust/climon-setup`, `rust/climon-update`, `rust/climon-install`; **feature branch** |
| cli-27 | Terminal control handoff (local surface) | The attached local terminal is the default controller of the shared PTY (controller id `"local"` for the in-process `climon <cmd>` host; `terminal-<pid>` for `climon attach`); when another surface takes control the terminal blanks behind a *"being viewed on a climon dashboard"* notice centered on the real console, swallows all input, and pressing **Space** takes control back, resizes the PTY to the terminal, and requests a fresh replay so it repaints immediately (never left blank on an idle screen). The reclaim repaint is rebuilt at the local terminal's own host dimensions from the bounded raw PTY replay (`render_screen_from_replay`), not from the controller-sized idle grid, so a narrower dashboard controller cannot permanently clip cells beyond its own right edge. Reclaim also synchronizes tracked mouse private modes (`1000`, `1002`, `1003`, `1005`, `1006`, `1015`, via `build_mouse_private_mode_restore_suffix`): every tracked mode is cleared first, then only the modes currently active in authoritative daemon state are re-enabled, so a physical terminal that missed a disable while suppressed cannot stay stuck reporting mouse events. Session teardown clears every tracked mouse mode once the PTY reader drains, so exiting while a dashboard is in control cannot leave the physical local terminal stuck in mouse-tracking mode. Both the displaced notice and the reclaim repaint clear only the *visible viewport* (never `\e[2J`) and reset SGR before every erase, so reclaiming preserves scrollback above the viewport and leaves no color bleed on the prompt. Space is the take-control key *only while displaced*; once the local terminal controls the grid, Space is ordinary shell input. (Ctrl+T was avoided because host terminal emulators and browsers commonly intercept it.) Displacement is decided by controller **identity**, not size — a smaller dashboard displaces the local terminal too (Windows retains an overgrown-ConPTY repaint deferral only as a restore-time rendering safety). When the controller disconnects the daemon falls control back by priority (`pwa` > `dashboard` > `terminal`, ties by recency). | Viewers never fight over terminal size — a displaced local terminal shows a clear message and is one keystroke away from reclaiming control, with no clipped columns and no leftover mouse-tracking garbage. | [manual-tests/terminal-control-handoff.md](manual-tests/terminal-control-handoff.md); `rust/climon-session/src/host.rs`, `rust/climon-session/src/fingerprint.rs`, `rust/climon-session/src/replay.rs`, `rust/climon-cli/src/client.rs`, `rust/climon-session/src/control.rs`, `rust/climon-proto/src/frame.rs`; **branch `terminal-control-handoff`** |
| cli-28 | Jiggle-repaint on local restore | When the local terminal regains control after being displaced, the daemon jiggles the PTY size one row and back so the wrapped command repaints its authoritative screen on top of climon's shadow-grid repaint. | Full-screen apps (htop, vim, less) show their true, current state the instant you reclaim the terminal — no stale or half-painted screen. | [manual-tests/jiggle-repaint-on-restore.md](manual-tests/jiggle-repaint-on-restore.md); `rust/climon-session/src/host.rs`; **branch `feat/jiggle-repaint-on-restore`** |
...
| srv-05 | Health / version endpoint | Unauthenticated `GET /health` reports `{ ok, version, remotesEnabled, ports, shortcuts, preferences }` (plus `features` on loopback requests). | A simple liveness/version probe for scripts, monitoring, and cross-OS discovery. | [manual-tests/phase01-client-server-split.md](manual-tests/phase01-client-server-split.md); `src/server/server.ts` |
...
| ID | Feature | What it does | Value add | Identified by |
|---|---|---|---|---|
| srv-15 | Smart notification snippets | Extracts a clean ≤160-char snippet of the last relevant terminal paragraph (skipping UI chrome, hint bars, spinners) and uses it as the notification/toast body. | Notifications tell you *what* the session is asking — the agent's actual question — instead of a generic "needs attention", so you can triage without opening it. | `feature.smartNotifications`; [manual-tests/smart-notifications.md](manual-tests/smart-notifications.md); capture in `rust/climon-session`, delivery in `src/server/push/`; **dev branch** |
...
| srv-17 | Self-managed host ingest tunnel | When host remotes are enabled, the dashboard server auto-ensures a stable Microsoft dev tunnel for the ingest daemon at startup (reuse if present, else create): tunnel id `climon-ingest-<slug>` derived from the anonymous `install.id`, labeled `climon-ingest`, with a non-secret display-only JSON description. The remotes dialog no longer creates/recreates tunnels; the manual `remote.tunnelId` flow still works. | A home host becomes discoverable by devboxes without copying a tunnel id, and reuses the same stable tunnel across restarts. | `feature.remotes`; [manual-tests/phase18-remote-host-tunnel.md](manual-tests/phase18-remote-host-tunnel.md); `src/remote/tunnel.ts`, `src/remote/ingest-tunnel-id.ts`, `src/server/server.ts`; **dev branch** |
...
| dash-22 | Terminal emoji / wide-character width fidelity | Loads xterm.js's Unicode 11 addon and a small custom provider so wide emoji render at two cells: Unicode 11 widths for plain wide emoji, plus VS16 emoji-presentation promotion (`❤️`, `⚠️`, `▶️`), keycaps (`1️⃣`), and skin-tone joining (`👍🏽`). ZWJ sequences (families/professions) are intentionally left unchanged. | Emoji-heavy terminal output stays aligned in the dashboard — no swallowed spaces or ghost glyphs for the common emoji cases. | [manual-tests/terminal-emoji-width.md](manual-tests/terminal-emoji-width.md); `src/web/components/TerminalView.tsx` (`configureTerminalUnicode`, `createEmojiUnicodeProvider`); **dev branch** |
| dash-23 | Terminal control handoff (dashboard/PWA) | Each browser/PWA surface attaches with a unique, robustly-generated `viewerId` (secure `crypto.randomUUID` with a `getRandomValues`/time fallback for non-secure http:// LAN origins) plus its `kind`, and is the controller only when the daemon's broadcast `controllerId` matches; **every non-controller surface is displaced** (blank *"being viewed elsewhere"* overlay + **Take control** button, all keystrokes swallowed) regardless of size. Actively choosing a session — clicking it in the desktop list or tapping **Open terminal** on mobile/PWA — automatically takes control and resizes the shared PTY to that surface (no session-list maximize button). A surface also **auto-takes control when its window regains focus or visibility** (edge-triggered `focus`/`visibilitychange`, skipped when already controlling, so it never fights while backgrounded). The displaced overlay is **deferred** (`DISPLACED_OVERLAY_DELAY_MS`) and cancelled on control (re)gain, so a take-control handshake never flashes the "viewed on another dashboard" dialog. The controller re-fits **only** on the displaced→controlling transition (`shouldRefitOnControlFrame`) and dedupes identical resize reports (`shouldSendResize`), so a surface never re-fits in response to the grid changes it itself caused — preventing the `resize→control→refit→resize` feedback storm that otherwise corrupts a mobile PWA whose viewport fit jitters. Focusing the terminal repaints stale glyphs but **never refits** (`repaintActiveTerminal` on `onFocusCapture`/click; the refresh+refit helper is reserved for the post-replay settle), so xterm's own focus churn (it re-focuses its helper textarea on every refresh/resize) cannot re-fire `focusin → refit → resize` in an unbounded shrinking spiral when a desktop dashboard takes control. The terminal's flex wrapper and root panes carry `minWidth: 0`, so the flex chain can actually shrink to a narrower dashboard pane before `FitAddon` measures the canvas — without it a stale, wider xterm canvas held its intrinsic flex width and overflowed off the right edge. Because hiding the page (tab switch, app backgrounding) tears down the terminal WebSocket before the native `visibilitychange` handler runs, `App` arms takeover for the currently viewed session (`armTakeControl`) immediately in that handler, ahead of React's reattachment; `TerminalView` queues the request and flushes it once the replacement socket opens, so the returning surface still reclaims control instead of missing the handshake. | Open the same session on your desktop and phone at once without them fighting over size — switch back to a window and it grabs control automatically, with no flicker or resize storms, and each open dashboard is uniquely identified so multiple tabs never collide. | [manual-tests/terminal-control-handoff.md](manual-tests/terminal-control-handoff.md); `src/web/control-state.ts`, `src/web/components/TerminalView.tsx`, `src/web/components/SessionItem.tsx`, `src/web/App.tsx`; **branch `terminal-control-handoff`** |
...
| ID | Feature | What it does | Value add | Identified by |
|---|---|---|---|---|
| pwa-01 | Installable app + offline cold boot | Service worker precaches the boot assets `/assets/app.js` and `/assets/xterm.css` (deliberately **not** the `/` document) and serves them network-first with cache fallback; top-level navigations always pass through, and HTML/opaque responses are rejected so a dev-tunnel login page can't poison the cache. | Install climon on your home screen; its JS/CSS load fast and survive brief offline blips, while a cold relaunch still re-authenticates an expired tunnel correctly. | `src/web/sw.ts`, `src/web/pwa/swCache.ts` |
...
| pwa-06 | Zoom lock & no overscroll | Disables pinch-zoom and constrains the page to a 1:1 visual-viewport layout (`overflow: hidden`). | A stable, app-like feel on mobile — the terminal doesn't accidentally zoom or bounce while you interact. | [manual-tests/pwa-zoom-lock.md](manual-tests/pwa-zoom-lock.md); `src/web` |
