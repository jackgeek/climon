...
| cli-27 | Terminal control handoff (local surface) | The attached local terminal is the default controller of the shared PTY; when a larger surface takes control the terminal blanks behind a *"being viewed on a climon dashboard"* notice, swallows all input, and **Ctrl+T** takes control back and resizes the PTY down to the terminal. When the controller disconnects the daemon falls control back by priority (`pwa` > `dashboard` > `terminal`, ties by recency). | Viewers never fight over terminal size — a too-small local terminal shows a clear message and is one keystroke away from reclaiming control. | [manual-tests/terminal-control-handoff.md](manual-tests/terminal-control-handoff.md); `rust/climon-cli/src/client.rs`, `rust/climon-session/src/control.rs`, `rust/climon-proto/src/frame.rs`; **branch `terminal-control-handoff`** |
...
| dash-23 | Terminal control handoff (dashboard/PWA) | Each browser/PWA surface reports its `viewerId`+`kind` and **follows** the single controller's grid when large enough, or is **displaced** (blank *"viewed at a larger size elsewhere"* overlay + Take control button, all keystrokes swallowed) when too small; a maximize (`ArrowMaximize`) button on non-controller surfaces takes control and resizes the shared PTY to fit. | Open the same session on your desktop and phone at once without one shrinking the other — take control from whichever device you're actually using with one button. | [manual-tests/terminal-control-handoff.md](manual-tests/terminal-control-handoff.md); `src/web/control-state.ts`, `src/web/components/SessionItem.tsx`, `src/web/components/TerminalView.tsx`; **branch `terminal-control-handoff`** |
