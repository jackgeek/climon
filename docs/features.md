...
| cli-27 | Terminal control handoff (local surface) | The attached local terminal is the default controller of the shared PTY and attaches with a unique `viewerId` (`terminal-<pid>`); when another surface takes control the terminal blanks behind a *"being viewed on a climon dashboard"* notice, swallows all input, and **Ctrl+T** takes control back, resizes the PTY to the terminal, and requests a fresh replay so it repaints immediately (never left blank on an idle screen). Displacement is decided by controller **identity**, not size. When the controller disconnects the daemon falls control back by priority (`pwa` > `dashboard` > `terminal`, ties by recency). | Viewers never fight over terminal size — a displaced local terminal shows a clear message and is one keystroke away from reclaiming control. | [manual-tests/terminal-control-handoff.md](manual-tests/terminal-control-handoff.md); `rust/climon-cli/src/client.rs`, `rust/climon-session/src/control.rs`, `rust/climon-proto/src/frame.rs`; **branch `terminal-control-handoff`** |
