...
| `remote.devtunnelProbeTimeout` | number | `5` | client | Seconds the launcher waits for the Dev Tunnels readiness probe (devtunnel --version + user show) before giving up and spawning the uplink best-effort. A stalled devtunnel is terminated on timeout so it never hangs launch. Raise it on slow networks; minimum 1. |
