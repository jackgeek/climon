...
| DAR-04 | 2026-07-19 | Copilot CLI | Linux x64 (WSL2) | 3.2.0 (`68bc1cf7`) | Partial | Real terminal + real browser. `vim` launched full-screen in the local terminal at 120×40 (via `pty-mcp-server`). Resized the Playwright browser viewport larger (1600×1000) *before* clicking **Take control**, displacing the local terminal; vim's full-screen buffer re-rendered correctly at the larger grid in the browser. Pressed **Space** locally to reclaim/restore: vim repainted cleanly (tildes filled the 120-col local width, no stale/half-painted screen). For the same-size take-control case (step 4), `htop`/`copilot` were unavailable in this sandbox, so `vim` was substituted. This verifies the daemon-level same-size jiggle-repaint path but does **not** exercise the required Ink/Copilot-style frame-caching behavior; that coverage remains outstanding. |
...
| DAR-09 | 2026-07-19 | Copilot CLI | Linux x64 (WSL2) | 3.2.0 (`68bc1cf7`) | Partial | Unix SIGINT/SIGTERM passed. Resolved the detached host each time via `pgrep -f "__session <id>"` (never `daemonPid`), verified a single match, then signaled only that PID. `kill -INT <host>` and `kill -TERM <host>` each produced prompt terminal failure metadata and released the socket. A same-PID double-signal race was not captured because the host reaped before the second signal. DAR-03/DAR-04 indirectly exercised attached resize/reflow through a real tty, but this DAR-09 run did not directly isolate and verify the SIGWINCH signal path, so that sub-case remains outstanding. |
...
The Linux release gate is **not passed**. DAR-01, DAR-02, and DAR-03 passed
using a real controlling terminal (via `pty-mcp-server`) and a real browser
(via Playwright/Chromium). DAR-04 is Partial because substituting Vim did not
exercise the required Ink/Copilot-style frame-caching case. DAR-09 is Partial:
SIGINT/SIGTERM passed and real local resize exercised reflow indirectly, but
the SIGWINCH path was not directly isolated in that case. DAR-08 is still a
**hard fail**, and the finding is now stronger: a reproducible
...
engine's default; once fixed, re-run DAR-08, DAR-04 with the intended
frame-caching TUI, and the direct DAR-09 SIGWINCH sub-case.
