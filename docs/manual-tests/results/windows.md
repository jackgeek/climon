...
| DAR-03 | 2026-07-22 | Copilot CLI | Windows x64 / Windows Terminal 1.24 + OpenConsole | 3.2.0 (`98bb7e1a`; SHA-256 `E454BC34278925C6E29F7A33A02A544F50A68243EA7FB50020D79A0F972E3965`) | Pass | Exact-candidate actors `thick-toes-pull` and repeat `poor-candies-brake` passed local dashboard ↔ Tunnel Link (`quick-shoe-3d10rcj.euw`, `lrzb71qj-3135.euw.devtunnels.ms`; access token omitted) handoff without fresh child output: colored history and the local marker appeared immediately in the tunnel surface, the tunnel marker appeared immediately after local reclaim, scrolling reached line 001 and returned to the tunnel marker, and non-controller displaced input was rejected. Both DevTools captures contained zero `[handoff-debug:*]` messages. Exact-candidate attached actor `modern-wasps-clean` ran in a real 120x30 Windows Terminal/OpenConsole surface: dashboard takeover showed the displaced notice; a foreground Win32 `X` keyboard event was swallowed with the notice unchanged; a foreground Space keyboard event restored the prompt without inserting input, displaced the dashboard, and restored metadata to 120x30. |
