...
SSE, and destructive session cleanup.
...
- **Preconditions:** Start the dashboard locally and note its port (for example,
  ...
  completed session with scrollback is best for the scrollback check.
...
4. Run `curl -i -X DELETE -H "Host: evil.com:$PORT" "http://127.0.0.1:$PORT/api/sessions/$SESSION_ID?kill=none"`.
5. Confirm the session still exists in the dashboard or with a normal loopback
   `GET /api/sessions` request.

**Expected:** Every hostile-Host request returns HTTP 403 before any session JSON,
scrollback bytes, SSE stream, or delete side effect is produced. The session
...
2. If a Tunnel Link is active, open the dashboard through the `*.devtunnels.ms`
   URL and confirm the session list and SSE updates still load.
...
for the session list/SSE flow; only non-loopback, non-`*.devtunnels.ms` Hosts
are rejected by the allowed-Host guard.
