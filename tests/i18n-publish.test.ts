...
const CATALOG: Catalog = {
  "telemetry.seed": { id: "0001a2b3", t: "telemetry catalog initialized", params: {} },
  "daemon.probe": {
    id: "ffaa0011",
...
describe("toLookupRows", () => {
  test("flattens each entry into id/key/template/params/redacted", () => {
...
describe("toCsv", () => {
  test("emits a header row followed by one row per entry", () => {
    ...
    expect(lines[0]).toBe("id,key,template,params,redacted");
  ...
  test("quotes and escapes fields containing commas or quotes", () => {
    ...
    expect(line).toBe('00000001,tricky,"say ""hi"", then {name}",name,');
  });
