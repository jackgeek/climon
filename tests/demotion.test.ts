...
describe("demote primitive", () => {
  test("runs closeListener, spawnUplink, stopLocalServer, removeBeacons — in order", async () => {
    const calls: string[] = [];
    await demote({
    ...
    expect(calls).toEqual(["closeListener", "spawnUplink", "stopLocalServer", "removeBeacons"]);
  });
