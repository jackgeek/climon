...
export function parseSetupOptions(args: string[]): SetupOptions {
  const options: SetupOptions = { apply: false };
...
/**
 * Runs the onboarding flow: telemetry opt-in, auto-update opt-in, and install-id
 * assignment. Both opt-ins default OFF. Onboarding always completes.
 */
