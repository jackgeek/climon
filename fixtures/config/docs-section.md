...
| `session.ipcTransport` | string | `local` | client, daemon | IPC transport used by session daemons. Defaults to owner-only local sockets/pipes; set to 'tcp' for authenticated loopback TCP when local socket paths are unavailable. |
