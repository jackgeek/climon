...
- **Never squash merge `dev` into `main`.** The `dev` → `main` release PR must use a real merge commit (`gh pr merge --merge`), never `--squash` or `--rebase`, so `main` and `dev` keep a shared ancestor and the next release diff stays clean. Squashing `dev` into `main` collapses hundreds of commits into one and makes `main`/`dev` diverge permanently.
- **When cutting a release, bump the version with `bun run release` (or bump `package.json` and the CLI fixtures in lockstep).** The golden CLI fixtures `fixtures/cli/version.txt` and `fixtures/cli/help.txt` embed `climon v<semver>` on line 1 and are asserted byte-for-byte by `rust/climon-cli/tests/cli_fixtures.rs`. `scripts/release.ts` rewrites all three together; if you edit `package.json` by hand you MUST also bump the version token on line 1 of both fixtures, or `cargo test` (and CI) goes red. Also watch for merge conflicts that resurrect an older version string in these fixtures when merging `dev` → `main`.
