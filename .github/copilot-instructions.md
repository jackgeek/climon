...
Session state is filesystem-backed under `$CLIMON_HOME` (default `~/.climon`): `config.json`, session metadata JSON, final scrollback, daemon logs, and sockets/pipes. Metadata is the cross-process coordination boundary; `src/store.ts` writes metadata atomically and serializes per-process patch bursts.
...
- Configuration is hierarchical for `climon config`: local `.climon/config.json` files are checked from the cwd upward before global `$CLIMON_HOME/config.json`; writes use explicit `--local`/`--global` or the nearest existing config.
